...
async function main() {
    ...
    const salon = await prisma.business.upsert({
        ...
        create: {
            ...
            name: 'Glamour Salon',
            nameAr: 'صالون جلامور',
            type: client_1.BusinessType.SALON,
            slug: 'glamour-salon',
    ...
    const clinic = await prisma.business.upsert({
        ...
        create: {
            ...
            type: client_1.BusinessType.CLINIC,
            slug: 'care-clinic',
    ...
    for (let i = 0; i < 5; i++) {
        ...
        await prisma.message.createMany({
            data: [
                { conversationId: conv.id, senderType: client_1.SenderType.CUSTOMER, messageType: 'TEXT', content: 'السلام عليكم، أبي أطلب', createdAt: new Date(Date.now() - 3600000) },
                { conversationId: conv.id, senderType: client_1.SenderType.BOT, messageType: 'TEXT', content: 'مرحباً! كيف يمكنني مساعدتك؟', metadata: { auto: true }, createdAt: new Date(Date.now() - 3500000) },
                { conversationId: conv.id, senderType: client_1.SenderType.CUSTOMER, messageType: 'TEXT', content: 'أبي دجاج مقلي', createdAt: new Date(Date.now() - 3400000) },
            ],
        });
    }
    // Auto replies
